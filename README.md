...
### 08. Прогнозное обслуживание двигателей (NASA Turbofan RUL Regression)
*   **Задача:**  Предсказание остаточного полезного ресурса (RUL) двигателей на основе временных рядов с датчиков (датасет NASA Turbofan FD003) с двумя видами поломки.
*   **Стек:** TensorFlow/Keras, Scikit-learn, LightGBM, NumPy, Pandas.
*   **Особенности**:
**Ансамбль моделей**: Сравнительный анализ и применение классического ML (Random Forest), градиентного бустинга (LGBM) и глубокого обучения (GRU).
**Кастомная оптимизация**: Использование специфической метрики S-score (асимметричный экспоненциальный штраф за завышенные предсказания RUL) и комбинированной функции потерь Safe Huber Loss для стабильного обучения GRU.
**Стабильность и валидация**: Внедрена полная фиксация random seed и методика Seed Averaging для борьбы с вариативностью инициализации весов и получения статистически достоверных результатов.

**Образ с настроенным окружением со всеми необходимыми библиотеками и GPU-поддержкой для запуска ноутбука**

Необходимо наличие nvidia-container-toolkit !
1. **Скачать образ:**
   ```bash
   docker pull sitnikne/predictive_maintenance_project:v1
